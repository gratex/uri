/*jshint esversion: 6 */
  let splitUriRegex = new RegExp( //IRI lib regexp
      "^" + "(([^:/?#]*):)?" + // scheme
      "(//((([^/?#@]*)@)?" + // user
      "(\\[[^/?#]*\\]|([^/?#:]*))?" + // host
      "(:([^/?#]*))?))?" + // port
    ...
    RFC3986_UNRESERVED = RFC2396_ALPHANUM + "\u002d\u002e\u005f\u007e",
    /* "-._~" */
    RFC3986_SUBDELIMS = "\u0021\u0024\u0026\u0027\u0028\u0029\u002a\u002b\u002c\u003b\u003d",
    /* "!$&'()*+,;=" */
    RFC3986_PCT_ENCODED = "", //
   ...
   **/
  const decomposeComponents = uriStr => {
    /*jshint maxcomplexity:50 */
    //ported
    let gs = uriStr.match(splitUriRegex);
    const u = {
        scheme: gs[2],
        authority: gs[4],
        path: gs[11],
   ...
   **/
  const recomposeComponents = uriObj => {
  ...
  const recomposeAuthorityComponents = (userInfo, host, port) => {
    if (host == null)
  ...
  const _checkAuthorityInvariant = uriObj => {
   ...
   **/
  let STRICT_TRANSFORMREFERENCES = true;

  const _transformReference = (base, ref) => {
    let t = {},
      r = ref;
  ...
  /**
   5.2.3.  Merge Paths
   **/
  const _merge = (base, refPath) => { //object,string
    if (base.authority != null && base.path === "") {
      return "/" + refPath;
    } else {
      let xi = base.path.lastIndexOf("/");
      if (xi == -1)
        return refPath;
      else
   ...
   **/
  export const percentEncode = (str, legalRange) => {

    let retVal = "",
      reLegal = legalRange != null ? new RegExp("[" + legalRange + "]") : null,
      i, enc, c;
    /**
  ...
  const decodeSegments = encodedPath => {
    // summary:
    //		Spliting path by "/".
    ...
    if (encodedPath === "")
      return [];
    let i, segments = encodedPath.split("/");
    if (segments.shift() !== "")
  ...
  export const encodeSegment = segment => percentEncode(segment, RFC3986_SEGMENT);
  const encodeQuery = str => percentEncode(str, RFC3986_QUERY);
  const encodeFragment = str => percentEncode(str, RFC3986_FRAGMENT);


  const checkEncoding = (raw, legalRange, doThrow /*, flags*/ ) => {
    // summary:
    ...
    if (!raw)
      return null;
    let reLegal = new RegExp("[" + legalRange + "]"),
      tokens = raw.match(PCHAR_TOKENIZER),
      i, e, t;
  ...
  const checkSegmentsEncoding = (str, doThrow) => checkEncoding(str, RFC3986_PATH_SEGMENTS, doThrow);
  const checkSegmentEncoding = (str, doThrow) => checkEncoding(str, RFC3986_SEGMENT, doThrow);
  const checkQueryEncoding = (str, doThrow) => checkEncoding(str, RFC3986_QUERY, doThrow);
  const checkFragmentEncoding = (str, doThrow) => checkEncoding(str, RFC3986_FRAGMENT, doThrow);

  // extensions
  const parseQuery = (query, bDecode) => {
    // summary:
  ...
  const isSubordinate = (uriParent, uriSub, orSame) => {
    ...
    if (uriSub.authority != null && uriSub.authority != uriParent.authority)
      return false;
    let i = uriSub.path.startsWith(uriParent.path, 0);
